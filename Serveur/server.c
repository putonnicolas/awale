...
static void forfeit(Client *client)
{
  ...
  else
  {
    ...
    Client *opponent = currentGame->clients[0] != client ? currentGame->clients[0] : currentGame->clients[1];
    char message[2048];
    snprintf(message, sizeof(message),
...
static void play_awale(Client *client, ParsedMessage *props)
{
  ...
  {
    send_message_to_specific_client(*client, "The hole index should be between 1 and 6... try again.", 1);
    return;
  }
  ...
  {
    send_message_to_specific_client(*client, "The hole is empty... try again.", 1);
    return;
  }
  ...
  {
    short currentBoardIndex = ((holeIndex + i) / HALF_AWALE_BOARD_SIZE + clientIndex) % 2;
    short currentHoleIndex = (holeIndex + i) % HALF_AWALE_BOARD_SIZE;

    // increment hole
    int *currentHole = &(game->halfAwaleBoards[currentBoardIndex][currentHoleIndex]);
    (*currentHole)++;
...
void sendEndOfTurnMessage(Game *game, EndOfTurnMessageMode mode)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    {
      snprintf(buffer, sizeof(buffer), "| %2d ", game->halfAwaleBoards[bottom][j]);
      strncat(message, buffer, sizeof(message) - strlen(message) - 1);
    }
    strncat(message, "|\n", sizeof(message) - strlen(message) - 1);

    // Graines capturées (on écrit ligne par ligne pour éviter les grands snprintf)
    snprintf(buffer, sizeof(buffer), "\nGraines capturées :\n");
    strncat(message, buffer, sizeof(message) - strlen(message) - 1);

    snprintf(buffer, sizeof(buffer), "  %s : %d\n", client->name, game->capturedSeeds[i]);
    strncat(message, buffer, sizeof(message) - strlen(message) - 1);

    snprintf(buffer, sizeof(buffer), "  %s : %d\n", opponent->name, game->capturedSeeds[(i + 1) % 2]);
    strncat(message, buffer, sizeof(message) - strlen(message) - 1);
